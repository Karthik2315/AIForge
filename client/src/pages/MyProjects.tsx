...
import { LucideLoader2, Plus, Trash2Icon, TrashIcon } from 'lucide-react';
        ...
        {loading ? (
          ...
          <div className='py-10 min-h-[80vh]'>
            <div className='flex items-center justify-between px-5'>
              ...
              <button className='text-white flex items-center justify-center gap-1 bg-gradient-to-r from-indigo-500 to-purple-500 px-2 py-2 rounded-full cursor-pointer hover:scale-105 active:scale-95 transition-all duration-300'><Plus className='size-5 font-medium text-white'/> Create New</button>
            </div>
            <div className='flex flex-wrap gap-3.5 mt-8'>
              {projects.map((project) => (
                  (
                  <div onClick={()=>navigate(`/projects/${project.id}`)} key={project.id} className='relative group w-72 max-sm:mx-auto cursor-pointer bg-gray-900/60 border border-gray-700 rounded-lg overflow-hidden shadow-md group hover:shadow-indigo-700/30 hover:border-indigo-800/80 transition-all duration-300 h-78'>
                    ...
                    <div className='p-4 text-white bg-linear-180 from-transparent group-hover:from-indigo-950 to-transparent transition-colors'>
                      ...
                      <div onClick={(e) => e.stopPropagation()} className='absolute bottom-2 w-[85%] flex justify-between items-center mt-6'>
                        <span className='text-xs text-gray-500'>{new Date(project.createdAt).toLocaleDateString()}</span>
                        <div className='flex gap-1 text-white'>
                          <button onClick={() => navigate(`/preview/${project.id}`)} className='text-[12px] px-3 py-1.5 bg-white/10 hover:bg-white/15 rounded-md transition-all cursor-pointer'>Preview</button>
                          <button onClick={() => navigate(`/projects/${project.id}`)} className='text-[12px] px-3 py-1.5 bg-white/10 hover:bg-white/15 rounded-md transition-all cursor-pointer'>Open</button>
                        </div>
                      </div>
                    </div>
                    <div onClick={(e) => e.stopPropagation()}>
                      <Trash2Icon className='absolute top-3 right-3 scale-0 group-hover:scale-100 bg-white p-1.5 size-7 rounded text-red-500 text-xl cursor-pointer transition-all' onClick={() => deleteProject(project.id)}/>
                    </div>
                  </div>
                )
              ))}
